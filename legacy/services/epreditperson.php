...
if($test['id'] == ""){
	...
	$query = "UPDATE `erp_people` SET `fName`='{$fName}', `lName`='{$lName}', `sex`='{$sex}', `contact`='{$contact}', `dob`='{$dob}', `bloodGroup`='{$bloodGroup}', `native`='{$native}', `nativeAddress`='{$nativeAddress}', `photoUrl`='{$photoUrl}', `documentsList`='{$documentsList}', `role`='{$role}', `joinDate`='{$joinDate}', `joinBranch`='{$joinBranch}', `currentBranch`='{$currentBranch}', `bankDetails`='{$bankDetails}', `currentAddress`='{$currentAddress}', `isActive`='{$isActive}', `height`='{$height}', `weight`='{$weight}', `religion`='{$religion}', `emergencyName`='{$emergencyContact}', `emergencyContact`='{$emergencyPhone}' WHERE `id`='{$id}'";
	...
	{  
		...
		unlink("images/erp/people/".$id.".jpg");
		mysql_query("UPDATE `erp_people` SET `photoUrl`='' WHERE `id`='{$id}'");
	}
