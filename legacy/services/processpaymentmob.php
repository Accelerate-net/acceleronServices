...
//Amount Validation
$amount_check = mysql_fetch_assoc(mysql_query("SELECT `paidAmount` FROM  `zaitoon_orderlist` WHERE `orderID`='{$orderID}' AND `razorpay_order_id` ='{$razor_order_id_initial}'"));
...
mysql_query("UPDATE `zaitoon_orderlist` SET `paymentStatus`= 1,`transactionID`='{$paymentID}',`isVerified`= 1 WHERE `orderID`='{$orderID}' AND `razorpay_order_id` ='{$razor_order_id_initial}'");

//Clear Voucher
$voucher_check = mysql_fetch_assoc(mysql_query("SELECT `usedVoucher` FROM  `zaitoon_orderlist` WHERE `orderID`='{$orderID}'"));
if($voucher_check['usedVoucher'] != ""){
	mysql_query("UPDATE `z_vouchers` SET `isActive`=0,`usedDate`='{$todayNow}',`usedOrder`='{$orderID}' WHERE `code`='{$voucher_check['usedVoucher']}'");
}
