...
if($tokenid['outlet']){
	...
	$notFound = 0;
				...
				$pdf->SetTitle('Sales Ledger - '.$outletname);
				...
				$pdf->Ln(3);		
				$pdf->Cell(185,10,"SALES LEDGER",50,80,'R');
				$pdf->Ln(1);
	...
	if(strtotime($query_date) > strtotime($to)){
		break;
				...
				$query = mysql_query("SELECT * FROM zaitoon_orderlist WHERE outlet='{$outlet}' AND stamp='{$query_stamp}' AND isVerified='1' AND status='2' ORDER BY orderID");
				...
				$remarks = $info['transactionID'];
				
								
				if($mode == 'Cash'){
					$sum_cod = $sum_cod + $value;
				}
				else{
					$sum_online = $sum_online + $value;
				}
		...
		$grand_sum = $sum_online + $sum_cod;
		
		$pdf->Cell(180,0,"______________________________________________________________________________________________________________________");
		$pdf->Ln(8);
		$pdf->SetFont('helvetica','',11);
		$pdf->SetX(100);
		$pdf->Cell(120,0,"Total by Cash");
		$pdf->SetFont('helvetica','',13);
		$pdf->SetX(150);
		$pdf->Cell(120,0,"Rs. ".$sum_cod);
		$pdf->Ln(8);
		$pdf->SetFont('helvetica','',11);
		$pdf->SetX(100);
		$pdf->Cell(120,0,"Total by Online");
		$pdf->SetFont('helvetica','',13);		
		$pdf->SetX(150);
		$pdf->Cell(120,0,"Rs. ".$sum_online);
		$pdf->Ln(8);
		$pdf->SetFont('helvetica','B',11);
		$pdf->SetX(100);
		$pdf->Cell(120,0,"Grand Total");
		$pdf->SetFont('helvetica','B',13);		
		$pdf->SetX(150);
		...
		if($notFound == 0)
			die('<br><br><center><p style="font-size:21px; color: #e74c3c">There are no Completed Orders found between '.$from.' to '.$to.'</p><a href="https://www.zaitoon.online/manager/finance-ledger.html">BACK</a></center>');
		
		
				
		$pdf->Output('Ledger_'.$outlet.'_'.$fromFancy.'_to_'.$toFancy.'.pdf', 'I');
