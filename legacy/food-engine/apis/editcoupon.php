...
if($_POST['rule'] == "PERCENTAGE"){
	...
	//Delete and Add it as new entry
	if(isset($_POST['permanentCode']) || $_POST['permanentCode'] != ""){
		mysql_query("DELETE FROM `z_couponrules` WHERE `code`='{$_POST['permanentCode']}'");
		$get_date = mysql_fetch_assoc(mysql_query("SELECT `from` FROM `z_couponrules` WHERE `code`='{$_POST['permanentCode']}'"));
		$createdday = $get_date['from'];
	}
	mysql_query("INSERT INTO `z_couponrules`(`code`, `rule`, `list`, `from`, `to`, `brief`, `limit`) VALUES ('{$coupon_code}', '{$myrule}', '{$final_list}', '{$createdday}', '{$_POST['expiry']}', '{$_POST['brief']}', '{$_POST['limit']}')");
