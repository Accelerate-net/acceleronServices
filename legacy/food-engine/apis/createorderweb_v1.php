...
$output = array(
		"status" => false,
		"error" => "The Version of the App you are using is no more supported since 7th September 2018. Please update the App to place an Order."
);
		...
		if($isTakeAway == false)
		{
				...
				else
				{
					if($carttamper == 1 || $total != $cart['cartTotal']){
						...
						{
							mysql_query("UPDATE `z_vouchers` SET `isActive`=0,`usedDate`='{$dateStamp}',`usedOrder`='{$orderid}' WHERE `code`='{$original_coupon}'");							
						}
					}
				}
		}
...
if($taxcheck['isParcelCollected']){
	if($isTakeAway){
		$extra_sum = $extra_sum + round($total*$taxcheck['parcelPercentagePickup'], 2);
	}
	else{
		$extra_sum = $extra_sum + round($total*$taxcheck['parcelPercentageDelivery'], 2);
	}
}

$grand_sum = $total + round($extra_sum) - $coupon_discount;
...
mysql_query("UPDATE `zaitoon_orderlist` SET `paidAmount`='{$grand_sum}' WHERE `orderID`='{$orderid}'");
