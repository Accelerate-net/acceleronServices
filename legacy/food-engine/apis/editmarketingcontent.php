...
if($co_type == 'promotion'){
	...
	//	die(json_encode($result));
	//}
	$brief = 'Welcome the New Year with us. We invite all of you for the Grand Dinner Buffet on 31st. Get 10% OFF when you pre book through our App.';
	$expiry = '3-6-2018';
	$type = 'purchase';
	$id = '105';
	$name = 'New Year Offer';
	...
	mysql_query("UPDATE `z_deals` SET `brief`='{$brief}',`validFrom`='{$today}',`validTill`='{$expiry}',`name`='{$name}',`maxCount`='{$maxCount}',`isActive`='{$isActive}',`price`='{$price}' WHERE `id`='{$id}'");
