...
$orderCheck = mysql_fetch_assoc(mysql_query("SELECT * FROM `smart_master_orders` WHERE `branch` = '{$outlet}' AND is_active = 1 AND order_status != 2 AND steward_code = '{$steward}' AND master_order_id = '{$master_order_id}'"));
$isFailed = true;
if($orderCheck['master_order_id'] == $master_order_id){
    ...
    if($subOrderCheck['order_id'] == $sub_order_id){
        mysql_query("UPDATE `smart_orders` SET `status`= 5 WHERE `order_id` = '{$sub_order_id}'");
        $isFailed = false;
    }
}
